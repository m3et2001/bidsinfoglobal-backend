...
import { tenders_all_field } from "./app/features/tenders/tenders.controller.js";
import { differenceInDays, differenceInSeconds, format, formatISO } from "date-fns";
import { sendDataMail } from "./app/features/auth/auth.controller.js";
import { readAllRegions, readAllSectors } from "./app/features/masters/masters.service.js";
...
import { ADMIN_EMAIL, BASE_URL } from "./app/helpers/constance.js";
        ...
        for (let index = 0; index < customerData.result.length; index++) {
            ...
            if (element.plans && element.plans.plan_expire_date) {
                ...
                if (notExpired) {
                    ...
                    if (element.tenders_filter.keywords && element.tenders_filter.keywords !== "")
                        filter = {
                            ...filter,
                            $or: [
                                { "description": { $regex: element.tenders_filter.keywords, $options: 'i' } }
                            ]
                        };

                    if (element.tenders_filter.cpv_codes && element.tenders_filter.cpv_codes.length > 0)
                        filter.cpv_codes = { $in: element.tenders_filter.cpv_codes }
                    if (element.tenders_filter.sectors && element.tenders_filter.sectors.length > 0)
                        filter.sectors = { $in: element.tenders_filter.sectors };
                    if (element.tenders_filter.regions && element.tenders_filter.regions.length > 0)
                        filter.regions = { $in: element.tenders_filter.regions };
                    if (element.tenders_filter.funding_agency && element.tenders_filter.funding_agency.length > 0)
                        filter.funding_agency = { $in: element.tenders_filter.funding_agency };

                    let tendersData = await readAllTenders(
                        filter,
                        tenders_all_field,
                        { ["createdAt"]: 1 },
                        0,
                    ...
                    // if (tendersData.result && tendersData.result.length > 0) {
        ...
        for (let j = 0; j < sectorData.result.length; j++) {
            ...
            for (let index = 0; index < header.length; index++) {
                ...
                if (index > 0) {
                    ...
                    let tenders = yesterdayTenderData.result.filter((obj) => obj.sectors === jElement.name && obj.regions === element.title);
                    string += `Tenders - ${tenders.length || 0}`;
                    let projects = yesterdayProjectsData.result.filter((obj) => obj.sectors === jElement.name && obj.regions === element.title);
                    string += ` / Projects - ${projects.length || 0}`;
                    let grants = yesterdayGrantsData.result.filter((obj) => obj.sectors === jElement.name && obj.regions === element.title);
                    string += ` / Grants - ${grants.length || 0}`;
                    let contract = yesterdayContractData.result.filter((obj) => obj.sectors === jElement.name && obj.regions === element.title);
