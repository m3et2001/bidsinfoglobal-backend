...
import {
  tendersAllList,
  tenders_all_field,
} from "../tenders/tenders.controller.js";
...
export const getCustomerById = async (req, res, next) => {
  try {
    ...
    const customerCheck = await readCustomers(
      { customer_id },
      activation_panel_filed
    );
    if (!customerCheck) throw new Error("Customer not found");
  ...
  try {
    ...
    if (req.body.purchase_plan_id) {
      ...
      sendEMAIL(
        ...
        `<p>Your plan ${
          plans?.title
        } has been activated today and will expire on ${format(
          end_date,
...
export const customerRegister = async (req, res, next) => {
  try {
    ...
    const customerUsernameCheck = await readCustomers(
      { username },
      { customer_id: 1 }
    );
    ...
    const filePath = path.join(
      __dirname,
      "/template/registrationMailFormat.hbs"
    );
    ...
    console.log("SAdfdsssssssssssssssssss");
    sendEMAIL(mailTo, "You are now registered with us", "", htmlToSend);

    responseSend(
      res,
      201,
      "You are now registered with us, our team will contact you and activate your account.",
      response
    );
  } catch (error) {
    next(error);
...
export const updateCustomerPassword = async (req, res, next) => {
  try {
    ...
    await updateCustomer(
      { customer_id },
      { ...customerCheck, last_password_reset: new Date() }
    );
...
export const resetCustomerPassword = async (req, res, next) => {
  try {
    ...
    if (!customerCheck)
      throw new Error("Invalid link or link expired to change password");
...
export const customerLogin = async (req, res, next) => {
  try {
    ...
    if (
      ...
      throw new Error(
        "Please wait or contact our team to active your account."
      );
...
export const customerForgotPassword = async (req, res, next) => {
  try {
    ...
    const customer = await readCustomers(
      { username },
      { email: 1, reset_password_link: 1 }
    );
    if (!customer) {
    ...
    const filePath = path.join(
      __dirname,
      "/template/forgetPasswordMailFormat.hbs"
    );
    const source = fs.readFileSync(filePath, "utf-8").toString();
    const template = handlebars.compile(source);
...
export const customersStatusUpdate = async (req, res, next) => {
  try {
    ...
    const customerCheck = await readCustomers(
      { customer_id },
      { full_name: 1, email: 1, username: 1, password: 1 }
    );
    if (!customerCheck) throw new Error("Customer not found");
    ...
    responseSend(
      ...
      201,
      `Customer ${
        req.body.status === "active" ? "activated" : "deactivated"
      } successfully`,
      {}
    );
  } catch (error) {
...
export const sendDataMail = async (
  tendersData = [],
  customer_id,
  full_name,
  email,
...
export const sendCaDataMail = async (
  CaData = [],
  customer_id,
  full_name,
  email,
...
export const assignTendersToCustomer = async (req, res, next) => {
  try {
    ...
    if (filter.data_type === "tender") {
      ...
      let customerData = await readCustomers(
        { customer_id },
        { contract_awards_filter: 1, contract_awards_id: 1 }
      );
      delete filter.data_type;
      ...
      let customerData = await readCustomers(
        { customer_id },
        { projects_filter: 1, projects_id: 1 }
      );
      delete filter.data_type;
...
export const usersList = async (req, res, next) => {
  try {
    ...
    if (keywords && keywords !== "")
      filter = {
        ...filter,
        $or: [{ username: { $regex: keywords, $options: "i" } }],
      };
