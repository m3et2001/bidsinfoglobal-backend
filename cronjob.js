...
import { tenders_all_field, tendersAllListForCron } from "./app/features/tenders/tenders.controller.js";
import { differenceInDays, differenceInSeconds, format, formatISO } from "date-fns";
import { sendCaDataMail, sendDataMail } from "./app/features/auth/auth.controller.js";
...
cron.schedule('*/10 * * * * *', async function () { // testing
    try {
        ...
        for (let index = 0; index < customerData.result.length; index++) {
            ...
            console.log(`Processing customer ${element.full_name} (ID: ${element._id})...`);
            ...
            if (element.plans && element.plans.plan_expire_date) {
                ...
                console.log(`No plan found or invalid plan for customer ${element.full_name} (ID: ${element._id}). Skipping.`);
            }
        }
